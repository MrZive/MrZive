 ...
 *
 */
public class FillMember386Plan2Data5 extends BaseDao{

	public static void main(String[] args) throws IOException, ParseException {
		MemberCard386Plan planCon = new MemberCard386Plan();
		planCon.setIs386Second(1);
		...
		for(MemberCard386Plan plan : list){
			...
			statusCon.setType(EnumType.is386Second);
			List<MemberPassStatus> conList = MemberStatusDao.getList(statusCon);
			if(conList.size()==0){
				MemberStatusDao.add(memberCard, EnumType.is386Second);
			}
		}
		getSession().commit();
	}
